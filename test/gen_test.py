# Генерирует 2-канальный (bit0=RX, bit1=TX) UART-поток, 8N1, 10 семплов/бит.
import struct
SPB = 10
def uart_bits(data):
    bits = []
...
rx = pkt(0x69, 0x25, [4] + list(b'0210')) + pkt(0x43, 0x05, [0, 0x89, 0xFE, 0x34, 0xE4, 0x18, 0x00]) \
   ...
   + pkt(0x69, 0x10, [1, 3, 0, 0x31, 0x32, 0x33]) + bytes([0x02, 0x43, 0x11, 2, 0, 0x99, 0, 1, 0x03])  # плохая CRC
